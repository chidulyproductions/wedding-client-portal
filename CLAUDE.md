...
- The brochure keeps the clean `song_title`, not the ID3 title — internal edit naming belongs on the player, not in the couple's program summary. The exception is an audio-only moment, which has no link to borrow a title from and so seeds the manual title field from the ID3 title.
